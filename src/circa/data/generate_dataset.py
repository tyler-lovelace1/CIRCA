...
def generate_dataset(num_group,
    ...
    while not stable_flag:
        # change random seed
        generator_seed = generator_seed + num_layer*100 + cnt*10000
        ...
        if dist_type == 'GRN':
            ...
            s = simulate_sergio(G, num_data, hill=6, mr_range=(0.25, 0.75), seed=generator_seed).T
        ...
        if num_layer > 0:
            ...
            mlplayer = [gen_mlp(num_dim=s_norm.shape[2],
                                num_layer=num_layer,
                                random_seed=generator_seed + k) for k in range(num_group)]
